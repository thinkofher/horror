...
func main() {
	errorsGo, err := os.Create("errors.go")
	die(err)
	defer errorsGo.Close()
	errorsGoTemplate.Execute(errorsGo, data)

	errorsTestGo, err := os.Create("errors_test.go")
	defer errorsTestGo.Close()
	errorsTestGoTemplate.Execute(errorsTestGo, data)
}
...
func {{ $element.Name }}(body []byte) Error {
	return Status(http.Status{{ $element.Name }}, body)
}
{{- end}}
`))

var errorsTestGoTemplate = template.Must(
	template.New("").Parse(`// Code generated by go generate; DO NOT EDIT.
// This file was generated at:
// {{ .Timestamp }}
package horror
